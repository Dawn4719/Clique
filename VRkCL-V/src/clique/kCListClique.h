...
private:
    ...
    std::vector<std::vector<int>> canAdd;
    std::vector<ui> canAddSize;
    std::vector<std::vector<int>> candidate;
    std::vector<ui> candidateSize;
    std::vector<std::pair<int, int>> gv;
    // std::vector<ui> deg;

    void buildSg(ui u);
    void listing(ui deep, ui edClique, int& lz);
...
public:
    ...
    unsigned long long run_v6();//core-peeling based
