    ...
    for (auto [d, u] : gv) {
        ...
        if (edges * 2 == nxtC.size() * (nxtC.size() - 1)) {
            if (k - 1 == nxtC.size() || k - 1 == 0) {
                ...
                // std::vector<int> vv;
                // vv.emplace_back(-1);
                // vv.insert(vv.end(), nxtC.begin(), nxtC.end());
                // // Node a(0, -1, vv);
                // st.tree.emplace_back(0, -1, vv);
                // std::string str = std::to_string(nxtC.size() + 1);
                // auto hashVal = fnv1a_64(str);
                //
                // // search(1, st.tree, 0, K - 1, 0, &answer);
                //
                // st.hashVal = hashVal;
                // if (!vertexHash.count(hashVal)) {
                //     vertexHash[hashVal] = ST.size();
                ...
                // }
                // st.init();
                // int minN = std::min(K - 1, (int)nxtC.size() - (K - 1));
...
// #pragma omp parallel private(n_edges) reduction(+:answer)
//     {
//         n_edges = 0;
// #pragma omp for schedule(dynamic, 1) nowait
    ...
    //  }
    // pt1 = (omp_get_wtime() - tt0) * 1e3;
    // pt2 = (omp_get_wtime() - tt1) * 1e3;
//     ST.clear();
